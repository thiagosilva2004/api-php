...
abstract class DAOAbstract
{
    ...
    public function apagar(int $id)
    {
        ...
        try {
            if ($this->db === null){
                throw new Exception("A instancia do banco nao foi passada", 1);
            } 
     ...
     * @return StdClass retornoRequisicao;
     */
    public function BuscarPeloID(int $id, array $campos = array())
    {
        $retornoRequisicao = RetornoRequisicao::getInstance();;
        try {
            if ($this->db === null){
            ...
            if(count($campos) <= 0){
                ...
                for ($i=0; $i < count($campos); $i++) { 
                    $campo = $i === 0 ? $campos[$i] : $campo . ' ,' . $campos[$i]; 
                }
            }

            $consulta = 'SELECT ' . $campo . ' FROM ' . $this->tabela . ' WHERE id = :id';
